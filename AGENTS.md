...
`@kurone-kito/is-prerelease` is a simple Node.js CLI that determines
whether a given semver version string is a pre-release, using the
[semver](https://www.npmjs.com/package/semver) package. It ships as a
single binary (`dist/index.mjs`) built with Vite and tested with
Vitest.
...
This runs `tsc -p tsconfig.test.json` (type-checking) and
`vitest run --coverage` (the unit test suite, `src/index.spec.mts`).
...
Runs `vite build`, producing the published CLI entry point at
`dist/index.mjs`.
...
- **Test location**: `src/index.spec.mts`, colocated with the source
  file it covers (`src/index.mts`)
- **Test runner**: [Vitest](https://vitest.dev/), with `@vitest/coverage-v8`
  for coverage reporting
- **Coverage target**: this package currently maintains 100% statement,
  branch, function, and line coverage; keep it there when adding code
- **Type checking**: `tsc -p tsconfig.test.json` runs alongside Vitest as
  part of `pnpm run test`, so a change must type-check as well as pass
  its tests
- **Test naming**: use descriptive names that explain the expected
  behavior (e.g., `it('should exit with 0 when version=%s and pre=%s', ...)`)
- **CI integration**: `.github/workflows/push-feature.yml` runs the full
  test suite across a Node.js (22.x/24.x/26.x) and OS/shell matrix on
