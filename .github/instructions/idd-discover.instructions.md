...
2. If the target issue carries the configured roadmap label or an
   `is-prerelease-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `is-prerelease-blocked-by` markers) and its
...
NOT contain an `is-prerelease-roadmap-id` marker (not itself
a roadmap) or an `is-prerelease-blocked-by` marker, AND
...
`is-prerelease-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`is-prerelease-roadmap-id` marker. Retro-label a legacy
...
`<!-- is-prerelease-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `is-prerelease-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `is-prerelease-roadmap-id` or
  `is-prerelease-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `is-prerelease-roadmap-id` marker when checking
  `is-prerelease-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- is-prerelease-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- is-prerelease-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- is-prerelease-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- is-prerelease-effort: S|M|L -->` footer
...
- **Roadmap identity** (`is-prerelease-roadmap-id`): in the
...
- **Sequential dependency** (`is-prerelease-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `is-prerelease-blocked-by` to group sub-tasks under
